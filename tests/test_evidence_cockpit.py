...
def test_evidence_cockpit_api_and_page():
    server_text = (ROOT / "oracle_server.py").read_text(encoding="utf-8", errors="replace")
    page_text = (ROOT / "ui" / "evidence.html").read_text(encoding="utf-8", errors="replace")
    assert '@app.get("/api/evidence-cockpit")' in server_text
    assert '@app.post("/api/evidence-cockpit/turn")' in server_text
    assert '@app.get("/evidence", response_class=HTMLResponse)' in server_text
    assert "Evidence Cockpit" in page_text

    canonical_root = Path(r"C:\Oracle\ORACLE.AI-runtime").resolve()
