...
class NPCRuntime:
    ...
    def process_event(
        ...
        if "attack" in event.kind.lower() or "threat" in event.kind.lower():
            self.world_model.add_threat(
                event.description, event.magnitude, event.participants[0] if event.participants else "unknown",
        ...
        ctx = policy_ctx or PolicyContext(
            ...
            target_reachable=interacting_with is not None,
        )
